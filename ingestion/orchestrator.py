...
CHANGES:
  - Added _clean_raw_items() to filter out split instructions masquerading as items.
  - Keeps all existing split logic (Pattern A and Pattern B) unchanged.
"""
...
# ---------------------------------------------------------------------------
# Helper: detect split instructions in raw items
# ---------------------------------------------------------------------------
SPLIT_KEYWORDS = {
...
def _clean_raw_items(splits: List[BuyerSplit]) -> List[BuyerSplit]:
    ...
    cleaned = []
    for split in splits:
        filtered_items = [item for item in split.raw_items if not _is_split_item(item)]
        # Keep split even if no items? Only if buyer_name is not "default" (i.e., a real person)
        if filtered_items or split.buyer_name.lower() != "default":
            cleaned.append(BuyerSplit(buyer_name=split.buyer_name, raw_items=filtered_items))
    return cleaned
...
def _get_mongo_client():
    try:
        ...
        raise ImportError("pymongo required for DB lookup")
...
def orchestrate_order_processing(
    ...
    """
    # === NEW: Clean raw items to remove split instructions ===
    parsed_data.raw_splits = _clean_raw_items(parsed_data.raw_splits)

    prices   = _get_store_prices()
    store_id = input_meta.get("store_id", "store_001")

    # Resolve primary customer from DB
    phone = input_meta.get("customer_phone", "unknown")
    customer_info = resolve_customer_details(phone)
    primary = customer_info["name"]
    db_group = customer_info["group_members"]
    ...
    has_empty_split = any(len(split.raw_items) == 0 for split in parsed_data.raw_splits)
    ...
    if has_empty_split:
        ...
        for split in parsed_data.raw_splits:
            for item in split.raw_items:
                ...
                try:
                    print(f"[orchestrator] match: raw={item.name!r} -> canon={canon_name!r} unit={canon_unit!r}", flush=True)
                except Exception:
                    pass
